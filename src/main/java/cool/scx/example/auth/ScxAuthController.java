...
import cool.scx.example.auth.exception.*;
import cool.scx.example.user.User;
import cool.scx.example.user.UserService;
...
public class ScxAuthController {
    ...
    private final Logger logger = LoggerFactory.getLogger(ScxAuthController.class);
     ...
     *
     * @param ctx         ctx
     * @param loginDevice a {@link cool.scx.example.auth.DeviceType} object
     * @return token
     * @throws cool.scx.example.auth.exception.AuthException c
     */
    ...
    public Json login(@FromBody String username, @FromBody String password, RoutingContext ctx) {
        try {
            ...
            if (authException instanceof UnknownDeviceException) {
                return Json.fail("unknown-device");
            } else if (authException instanceof UnknownUserException) {
                return Json.fail("user-not-found");
            } else if (authException instanceof WrongPasswordException) {
                return Json.fail("password-error");
            } else if (authException instanceof MaximumAtLoginInSameTimeException) {
                return Json.fail("maximum-at-login-in-same-time");
            } else {
                logger.error("登录出错 : {}", authException.getMessage());
    ...
    @ScxMapping(method = HttpMethod.POST)
    public Json logout() {
        var b = ScxAuth.removeAuthUser(ScxContext.routingContext());
    ...
    public Json info() throws UnauthorizedException {
        var user = ScxAuth.getLoginUser(ScxContext.routingContext());
        //从session取出用户信息
        if (user == null) {
            ...
            return Json.ok()
                    ...
                    .put("perms", ScxAuth.getPerms(user))
     ...
     * @param password 密码
     * @return 登录成功的用户
     * @throws cool.scx.example.auth.exception.AuthException 登录失败的错误
     */

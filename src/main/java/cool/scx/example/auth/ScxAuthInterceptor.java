...
import cool.scx.example.auth.annotation.Perms;
 ...
 * @version $Id: $Id
 */
public final class ScxAuthInterceptor implements ScxMappingInterceptor {
    ...
    public void preHandle(RoutingContext context, ScxMappingHandler scxMappingHandler) {
        ...
        if (p.checkedLogin) {
            ...
            } else if (p.checkedPerms && !currentUser.isAdmin && !ScxAuth.getPerms(currentUser).contains(p.permStr)) {
    ...
    private AuthPerms getScxAuthPerms(ScxMappingHandler s) {
        ...
        if (p == null) {
            var scxMappingHandlerPerms = new AuthPerms(s.clazz, s.method);
